...
export default function StockSelector({ onSelectStock, defaultValue = "" }: StockSelectorProps) {
  ...
  useEffect(() => {
    if (stocks.length > 0 && defaultValue && !selectedStock) {
      const defaultStock = stocks.find((stock) => stock.symbol.toLowerCase() === defaultValue.toLowerCase())
      if (defaultStock) {
  ...
  const debouncedFilter = useCallback((term: string, stockList: Stock[]) => {
    ...
    debounceRef.current = setTimeout(() => {
      if (!term || term.trim().length === 0) {
        ...
        const popularStocks = stockList
          ...
          .concat(
            stockList.filter(
              (stock) => !["AAPL", "MSFT", "NVDA", "GOOGL", "AMZN", "TSLA", "META", "AMD"].includes(stock.symbol),
            ),
          )
          .slice(0, 50)
      ...
      {selectedStock && !isOpen && !error && (
        <div className="mt-2 p-3 bg-gradient-to-r from-gray-50 to-blue-50 rounded-lg border border-gray-200">
          <div className="flex items-center justify-between">
            ...
            <span
              className={`px-2 py-1 rounded-full text-xs font-medium flex-shrink-0 ml-2 ${getSectorColor(selectedStock.sector)}`}
            >
              ...
              {filteredStocks.map((stock, index) => (
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    <span
                      className={`px-2 py-1 rounded-full text-xs font-medium flex-shrink-0 ml-2 ${getSectorColor(stock.sector)}`}
                    >
                      {stock.sector}
                    </span>
                  </div>
